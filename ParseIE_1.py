...
def import_chunk(chunk):
    ...
    with open('./data/IE_1.csv', encoding='utf-8') as csv_file:
        ...
        for row in csv_reader:
            ...
            if line_count>0 and line_count in chunk:
                ...
                data = {}
                newClaims = []
                if not str(line_count+400) in dictionaryIdentifiers:
                    print("enter here")
                    wikibase_item = pywikibot.ItemPage(wikibase_repo)
                    data['labels'] = {"en": row[0][0:400].lstrip().rstrip()}
                    ...
                    claim.setTarget(str(line_count+400))
                ...
                # 14,235,902.00
                target = pywikibot.WbQuantity(amount=float(float(row[7])*float(row[8])), unit=wikibase_unit,site=wikibase_repo)
                ...
                wikibase_unit = pywikibot.ItemPage(wikibase_repo, "Q226")
                # 14,235,902.00
                target = pywikibot.WbQuantity(amount=float(row[7]), unit=wikibase_unit,
                                              site=wikibase_repo)
                claim.setTarget(target)
                newClaims.append(claim.toJSON())
                ...
                if row[2] != "":
                    ...
                    response = requests.get(
                        "https://nominatim.openstreetmap.org/search/search?q=" + urllib.parse.quote_plus(
                            row[2]) + "&format=json&addressdetails=1")
                ...
                if row[6]!="":
                    claim = pywikibot.page.Claim(wikibase_repo, "P33",
                                                 datatype='time')
                    date = datetime.strptime(row[6], "%Y-%m-%d").isoformat() + "Z"
                    target = pywikibot.WbTime.fromTimestr(site=wikibase_repo, datetimestr=date, precision=11)
                    claim.setTarget(target)
                    newClaims.append(claim.toJSON())
                ...
                target = pywikibot.WbQuantity(amount=float(row[8])*100, unit=wikibase_unit, site=wikibase_repo)
                claim.setTarget(target)
                newClaims.append(claim.toJSON())
                ...
                try:
                    ...
                    for i in range(1, 1000):
                        if fine == False:
                            try:
                                print(row[0] + "_" + str(i))
                                data['labels'] = {"en": row[0].lstrip().rstrip() + "_" + str(i)}
                                wikibase_item.editEntity(data)
                                fine = True
                            except pywikibot.exceptions.OtherPageSaveError as e:
                                fine = False
