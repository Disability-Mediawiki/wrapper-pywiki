...
def import_chunk(chunk):
    ...
    with open('./data/FR/FR_3_FSE.csv', encoding='utf-8') as csv_file:
        ...
        for row in csv_reader:
            ...
            if line_count in chunk and len(row[6])>900:
                    ...
                    if not id in dictionaryIdentifiers:
                        ...
                        for wikibase_claims in wikibase_item.claims:
                            for wikibase_c in wikibase_item.claims.get(wikibase_claims):
                                if wikibase_c.toJSON().get('mainsnak').get('property') == "P836" :
                        ...
                        if len(claimsToRemove) > 0:
                            ...
                            #     # 14,235,902.00
                            #     print(row[9].replace(",", ".").replace(" ",""))
                            #     target = pywikibot.WbQuantity(amount=float(row[9].replace(",", ".").replace(" ","")), unit=wikibase_unit,
                            #                                   site=wikibase_repo)
                            ...
                            #     target = pywikibot.WbQuantity(amount=row[12].replace(",", "."), unit=wikibase_unit, site=wikibase_repo)
                            #     claim.setTarget(target)
                            #     newClaims.append(claim.toJSON())
                            #
                            # if row[9] != "" and row[12] != "":
                            ...
                            #     wikibase_unit = pywikibot.ItemPage(wikibase_repo, "Q226")
                            #     # 24892,21
                            #     target = pywikibot.WbQuantity(amount=round(float(row[9].replace(",", ".").replace(" ",""))/100*float(row[12].replace(",", ".")),2), unit=wikibase_unit,site=wikibase_repo)
                            #     claim.setTarget(target)
                            #     newClaims.append(claim.toJSON())
                            ...
                            if row[6]!="":
                                ...
                                if text.replace("\n","").replace("\t","").lstrip().rstrip()!="":
                                    ...
                                    object = pywikibot.WbMonolingualText(text = text[0:5000].replace("\n"," ").replace("\t"," ").lstrip().rstrip(), language="fr")
                                    claim.setTarget(object)
                                    newClaims.append(claim.toJSON())
                            #
                            # #adding the program
                            ...
                            # print(row[3].replace("Programme Opérationnel National ","").lstrip().rstrip())
                            # if row[3].replace("Programme Opérationnel National ","").lstrip().rstrip() in dictionaryFondIdentifiers and found == False:
                            #     # print("Not FOUND ",row[4])
                            #     claim = pywikibot.Claim(wikibase_repo, "P1584", datatype='wikibase-item')
                            #     object = pywikibot.ItemPage(wikibase_repo,
                            #                                 dictionaryFondIdentifiers[row[3].replace("Programme Opérationnel National ","").lstrip().rstrip()])
                            #     claim.setTarget(object)
                            #     newClaims.append(claim.toJSON())
